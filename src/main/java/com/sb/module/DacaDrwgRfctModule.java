...
public class DacaDrwgRfctModule {
	
	@Autowired
	IDacaDrwgRfctModule	iDacaDrwgRfctModule;
	
	public DacaDrwgRfctModuleOutVO dacaDrwgRfctModule(DacaDrwgRfctModuleInVO inData) throws Exception{
		...
		DacaDrwgRfctModuleOutVO out = new DacaDrwgRfctModuleOutVO();
		try
		{	
		    ...
		    ***********************************************************************/
			...
			s001MapOut = iDacaDrwgRfctModule.S001(s001MapIn);
			...
			/*2. 계좌잔고 갱신 */
			if(l_bfDaca - inData.getDrwg_amt() < 0) {
			...
			u001MapIn.put("DACA", l_bfDaca - inData.getDrwg_amt());
			u001MapIn.put("ACNO", inData.getAcno());
			
			rsCnt = iDacaDrwgRfctModule.U001(u001MapIn);
			
			if(rsCnt <=  0) {
				throw new Exception( iDacaDrwgRfctModule.getClass().getName()+"."+"U001"+" 처리 건수 없음.");
			}
			...
			s001MapOut2 = iDacaDrwgRfctModule.S001(s001MapIn2);
	 ...
	 * @throws Exception
	 */
	private void f_init_Check(DacaDrwgRfctModuleInVO inData) throws Exception {
		...
		if(inData.getDrwg_amt() <= 0) {
			throw new Exception("입력값 출금금액을 확인하세요");
		}
	}
}
